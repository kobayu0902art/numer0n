...
def one_point_three():
    ...
    while eat!=3:
        if eat == 3:
            break
        com,count=strategy(player)
        eat,bite,same=judge(player,com)
        timesthree+=1
    tthree=time.time()-tthree
    return tthree,timesthree,count
...
while count < 100:
    ...
    threetime,threetimes,innercount=one_point_three()
    totalinnercount=totalinnercount+innercount
